...
use utf8;
use strict;
our ($LOG, $LOAD, $opt_f, $opt_u, $opt_D, $opt_I, $opt_O, %MORPH);
if (0)
{
...
sub replace_morphology
{
    my($e) = @_;
    my($res, $eid);	
    $e =~ s|(<morphologyXref[ >].*?</morphologyXref>)|&split;&fk;$1&split;|gi;
    my @BITS = split(/&split;/, $e);
    my $res = "";
    foreach my $bit (@BITS){
	if ($bit =~ s|&fk;||gi){
	    my $subref = restructure::get_tag_attval($bit, "morphologyXref", "e:targeteltid");
	    my $morph = $MORPH{$subref};
	    unless ($morph =~ m|^ *$|)
	    {
		$bit = $morph;
	    }
