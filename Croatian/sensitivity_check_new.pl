...
our (%DEROG, %OFFENSIVE, %VULGAR, %SENSITIVE, %CLASSES, %DEF, %SENS, %TYPES, %CPDS, %FSCORE, %DSCORE, %SCORE, %EXCLUDE, %IGNORE, %REGISTER);
#
...
sub main
{
  ...
  line:    
	...
	foreach my $bit (@BITS){
		...
		{
		    my $def = $DEF{$wd};
		    my $register = $REGISTER{$wd};
		    my $wd_context = &get_wd_context($context, $wd);
		    my $wdscore = $SCORE{$wd};
		    my $info = &get_sensitivity_info($wd);
		    
		    &print_row($wd, $hw, $wd_context, $tag, $info, $def, $register, $lexid, $EntryId, $eid, $dbid, $wdscore, $totscore, $ct++); ## TO DO
		}
	    }
	    $res .= $bit;
	}	
...
sub load_file
{
  ...
  wloop:
	...
	$SENSITIVE{$wd} = 1;
	my ($wdfreq, $wddegree, $register);
	next wloop if (m| degree=\"low\"|);
	if (m| freq=\"(.*?)\"|)
	{
	    $wdfreq = $1;
	} else {
	...
	$register = restructure::get_tag_contents($_, "register"); 
