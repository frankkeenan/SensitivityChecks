...
#
#Calls
# $e = restructure::del_sup($e); # Del suppressed="true" elements
...
sub do_ndvs
{
	...
	if ($bit =~ s|&fk;||gio)
	{
	    ...
	    {
		my $xs = "";
		my $xhm = restructure::get_tag_contents($bit, "xhm"); 
		unless ($xhm =~ m|^ *$|)
		{
...
sub convert_to_ndvs
{
    ...
    foreach my $bit (@BITS){
	if ($bit =~ s|&fk;||gi){
	    my $type = restructure::get_tag_attval($bit, "xr-gs", "xt"); 
	    if ($type =~ m|ndv|i)
	    {
...
#    $e =~ s|<vs-g.*?</vs-g>||gi;
	...
	if ($bit =~ s|&fk;||gio){
	    ...
	    {
		my $wd = $1;
		my $f = &get_audio_file($bit);		
		if (&wd_comp($bold, $wd))
		{
		    my $wdf = sprintf("%s\t%s", $bold, $f);
		    unless ($FUSED{$wdf})
		    {
...
sub add_missing_attvals
{
    ...
    foreach my $bit (@BITS){
	if ($bit =~ s|&fk;||gi){
	    my $val = restructure::get_tag_attval($bit, $tagname, $attname);    
	    if ($attval =~ m|^ *$|)
	    {
