...
sub sup_example_groups
{
    my($e) = @_;
    my($res, $eid);	
    $e =~ s|(<exampleUnit[ >].*?</exampleUnit>)|&split;&fk;$1&split;|gi;
    my @BITS = split(/&split;/, $e);
    my $res = "";
    foreach my $bit (@BITS){
	if ($bit =~ s|&fk;||gi){
	    if ($bit =~ m|<translation[^>]* suppressed=\"true\"|)
	    {
		$bit = restructure::set_tag_attval($bit, "exampleUnit", "suppressed", "true"); 
	    }
	}
	$res .= $bit;
    }
