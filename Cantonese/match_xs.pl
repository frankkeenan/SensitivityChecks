...
use utf8;
use strict;
our ($LOG, $LOAD, $opt_f, $opt_u, $opt_D, $opt_I, $opt_O, $PDIR, %W, %USED, %F, %INFO, %EID);
#$PDIR = ".";
$PDIR = "/usr/local/bin/";
...
sub main
{
  ...
  line:    
	...
	my $x = restructure::get_tag_contents($_, "Context");
	my $frow = restructure::get_tag_contents($_, "FileRow"); 
	my $x2 = $x;
	$x2 =~ s|[^a-z0-9]||gi;
	my $eid = 	$EID{$x2};	
	if ($eid =~ m|^ *$|)
